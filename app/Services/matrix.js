...
Object.defineProperty(exports, "model", { enumerable: true, get: function () { return baseService_1.model; } });
class matrix extends baseService_1.default {
    async matrix(arr, mtr, opt) {
        const kat = arr;
        const mkat = mtr;
        const mat = [];
        for (let i = 0; i < kat.length; i++) {
            ...
            for (let j = 0; j < kat.length; j++) {
                ...
                item =
                    opt == "kat"
                        ? await baseService_1.model.mkategori.findOne({
                            where: { k1: kat[i].id, k2: kat[j].id },
                        })
                        : opt == "sub"
                            ? await baseService_1.model.msub.findOne({
                                where: { sub1: kat[i].id, sub2: kat[j].id },
                            })
                            : opt == "div"
                                ? await baseService_1.model.mdiv.findOne({
                        ...
                        opt == "kat"
                            ...
                            : opt == "sub"
                                ...
                                : opt == "div"
                                    ...
                                    : null;
                    if (item != null)
                        row.push(+parseFloat(`${1 / item.dataValues.nilai}`).toFixed(4) - 0);
                }
                else {
        ...
        const ntotal = [];
        for (let i = 0; i < kat.length; i++) {
            ntotal.push(+parseFloat(`${norm[i].reduce((a, b) => a + b)}`).toFixed(4) - 0);
        }
        ...
        const ci = +parseFloat(`${(lambda - kat.length) / (kat.length - 1)}`).toFixed(4) - 0;
        let ir;
        switch (kat.length) {
            case 1:
            case 2:
